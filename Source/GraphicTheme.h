...
{
public:
    CustomToggleButton(const juce::String& buttonName)
...
{
public:
    CustomLabel(const juce::String& labelName, const juce::String& text)
...
{
public:
    CustomComboBox(const juce::String& comboBoxName)
        : juce::ComboBox(comboBoxName)
    {
    }
...
public:
                          ...
                          bool shouldDrawButtonAsDown) override
    {
        ...
        const auto bounds = button.getLocalBounds().withSizeKeepingCentre(
            button.getWidth() - 8.0f, button.getHeight() - 2.0f);

        const auto backgroundColor
            = isOn ? juce::Colour(ZEROS_COLOUR) : juce::Colour(POLES_COLOUR);
        ...
        const auto indicatorX
            = isOn ? (bounds.getRight() - indicatorDiameter - 2)
                   : (bounds.getX() + 2);
        const auto indicatorY
            = bounds.getCentreY() - (indicatorDiameter / 2) + 0.5f;

        g.setColour(indicatorColor);
        g.fillEllipse(indicatorX, indicatorY, indicatorDiameter,
                      indicatorDiameter);
...
{
public:
    ActiveSwitchTheme() { backgroundColour = juce::Colour(SWITCH_BACKGROUND); }
                          ...
                          bool shouldDrawButtonAsDown) override
    {
        ...
        juce::Rectangle<float> switchRect(xOffset, yOffset, switchWidth,
                                          switchHeight);
        ...
        juce::Rectangle<float> buttonRect(
            xOffset + adjustedSwitchOffset, yOffset + adjustedSwitchOffset,
            switchWidth - adjustedSwitchOffset * 2,
            switchHeight - adjustedSwitchOffset * 2);

        g.setColour(isOn ? juce::Colour(ON_SWITCH_COLOUR)
                         : juce::Colour(OFF_SWITCH_COLOUR));
        g.fillRoundedRectangle(buttonRect, borderRadius);

        const juce::Colour ledColour
            = juce::Colour(isOn ? LED_ON_COLOUR : LED_OFF_COLOUR);
        ...
        juce::Rectangle<float> textRect(xOffset - 8, yOffset - 0.5, switchWidth,
                                        switchHeight);
        ...
        g.setColour(juce::Colours::black);
        g.drawEllipse(ledXOffset, ledYOffset, LED_RADIUS * 2, LED_RADIUS * 2,
                      1.5f);
    }
...
{
public:
    AutoUpdateSwitchTheme() { backgroundColour = juce::Colour(0xffD7DBDC); }
                          ...
                          bool shouldDrawButtonAsDown) override
    {
        ...
        juce::Rectangle<float> textRect(xOffset - 8, yOffset - 1, switchWidth,
                                        switchHeight);
        g.drawText(text, textRect, juce::Justification::centred);
...
public:
                          ...
                          bool shouldDrawButtonAsDown) override
    {
        ...
        auto bounds = button.getLocalBounds().withSizeKeepingCentre(
            button.getWidth() - 8.0f, button.getHeight() - 2.0f);
...
{
public:
    GenericButtonTheme(juce::String textToDisplay = "", float size = 11.0f)
        : text(textToDisplay), fontSize(size)
    {
    }
                              ...
                              bool shouldDrawButtonAsDown) override
    {
        auto bounds = button.getLocalBounds()
                          .withSizeKeepingCentre(button.getWidth() - 2.0f,
                                                 button.getHeight() - 2.0f)
                          .toFloat();
    ...
    void setTextToDisplay(juce::String textToDisplay) { text = textToDisplay; }
...
public:
    ComboBoxTheme()
    {
        ...
        setColour(juce::ComboBox::backgroundColourId,
                  juce::Colour(0xff909497).darker(0.2));
                      ...
                      int buttonH, juce::ComboBox& box) override
    {
        ...
        g.drawRoundedRectangle(boxBounds.toFloat().reduced(0.5f), cornerSize,
                               1.0f);
        ...
        arrowPath.addTriangle(0.0f, 0.0f, baseWidth, 0.0f, baseWidth * 0.5f,
                              arrowHeight);
        ...
        g.fillPath(arrowPath,
                   juce::AffineTransform::translation(arrowX, arrowY));
    }
                           ...
                           const juce::Colour* textColour) override
    {
        ...
        else
        {
            ...
            if (textColour != nullptr) textColourToUse = *textColour;
            ...
            g.drawFittedText(text, area.reduced(10, 0),
...
public:
    ...
    void mouseDrag(const juce::MouseEvent& event) override
    {
        ...
        double skewFactor
            = juce::jmap<double>(std::abs(yOffset), 0.0, 10000, 0.1, 4.0);
        auto newValue = initialValue + yOffset * skewFactor;

        setText(juce::String(newValue, 1) + " dB",
