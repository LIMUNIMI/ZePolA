...
public:
    ...
    // The constructor of FilterElement requires the specification of the element type (ZERO or POLE), while magnitude and phase have default values: 0.0 for both.
    ...
    // The method setMagnitude sets the magnitude of the complex number to the value of newValue and calls the calculateCoefficients method to compute the new coefficients for the difference equation.
    ...
    // The method setPhase sets the phase of the complex number to the value of newValue and calls the calculateCoefficients method to compute the new coefficients for the difference equation.
    ...
    // The method getMagnitude returns the magnitude of the complex number represented by FilterElement.
    double getMagnitude ()
    {
        return magnitude;
    }

    // The method getPhase returns the phase of the complex number represented by FilterElement.
    double getPhase ()
    {
        return phase;
    }

    // The method getRealPart returns the real part of the complex number represented by FilterElement.
    double getRealPart ()
    {
    ...
    // The memoryReset method resets the memory of the FilterElement (whether it is a ZERO or a POLE) to zero.
    void memoryReset ()
    {
        memory1 = 0.0;
        memory2 = 0.0;
    ...
    // The method calculateCoefficients calculates the coefficients to be used by the FilterElement in computing the output sample. To maintain the Hermitian symmetry of the filter, each zero and each pole have their conjugate. This means that the coefficient calculation, whether for a pole or a zero, reduces to the sum of the conjugates (twice the real part) and the product of the conjugates (the square of the magnitude).
    ...
    // The processBlock method takes the entire buffer of audio samples as input and calls the processSample method for each sample in the buffer.
    ...
    // The updateMemory method updates the filter's memory. memory1 stores the previous sample, which is an input sample if it is a ZERO or an output sample if it is a POLE. memory2 stores the sample from two instances ago, which is an input sample if it is a ZERO or an output sample if it is a POLE.
...
public:
    ...
    // The constructor of PolesAndZerosCascade has default values for the number of zeros and poles that the filter should have at the time of creation.
    ...
    // The memoryReset method resets the memory of the entire filter by calling the reset method for all the elements that make up the filter's cascade.
    void memoryReset ()
    {
    ...
    // The setUnsetBypass method bypasses the entire filter depending on the value of newValue chosen by the user as a parameter of the plugin.
    ...
    // The addElement method adds an element of the specified type to the cascade of zero or pole elements. The addition occurs at the end of the list of pointed elements.
    ...
    // The removeElement method removes an element of the specified type from the cascade of zero or pole elements. The most recently added element that matches the specified type is the one that gets removed.
    ...
    // The parameterChanged method allows the adjustment of bypass values as well as the magnitude and phase of each active zero and pole in the cascade. The parameter parameterID contains a string that identifies the parameter to be changed, and newValue contains the new value to be assigned.
