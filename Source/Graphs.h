...
public:
    ...
    void updateValues(double* vls, double* rf, const double sr, bool yType)
    {
        ...
        std::copy(rf, rf + NUMBER_OF_REFERENCE_FREQUENCIES,
...
protected:
    ...
    void drawGridAndReferenceFrequencies(juce::Graphics& g, bool linLog,
                                         const juce::Rectangle<float>& bounds)
    {
        std::vector<float> levels
            = linLog ? std::vector<float> {0.1f, 0.5f, 1.0f, 1.5f, 2.0f}
                     : std::vector<float> {-50.0f, -30.0f, -20.0f, -10.0f,
        ...
        {
            float yPos = juce::jmap<float>(level, linLog ? 0.0f : -60.0f,
                                           linLog ? 2.0f : 60.0f,
                                           bounds.getBottom(), bounds.getY());

            g.setColour(juce::Colour(TEXT_COLOUR));
            g.drawText(linLog ? juce::String(level)
                              : juce::String(level) + " dB",
                       5, yPos - (level == 2 ? 0 : 10), 40, 20,
                       juce::Justification::left);

            if (level != 2)
            {
                g.setColour(juce::Colour(GRID_COLOUR));
                g.drawHorizontalLine(static_cast<int>(yPos), bounds.getX(),
                                     bounds.getRight());
            }
        }
    }
...
public:
    ...
    void drawResponse(juce::Graphics& g, bool linLog) override
    {
        ...
        responsePath.startNewSubPath(
            bounds.getX(), calculateYValue(linLog, values[0], bounds));
        ...
        for (int i = 1; i < GRAPHS_QUALITY; ++i)
        {
            ...
            {
                g.setColour(juce::Colour(GRID_COLOUR));
                g.drawVerticalLine(static_cast<int>(x), bounds.getY(),
                                   bounds.getBottom());
                g.setColour(juce::Colour(TEXT_COLOUR));
                g.drawText(
                    formatFrequency(referenceFrequencies
                                        [i
                                         / (GRAPHS_QUALITY
                                            / NUMBER_OF_REFERENCE_FREQUENCIES)]
                                    * sampleRate),
                    x - 10, bounds.getCentreY(), 20, 20,
        ...
        g.setColour(juce::Colour(TEXT_COLOUR));
        g.drawText(formatFrequency(sampleRate * 0.5), bounds.getRight() - 20,
                   bounds.getCentreY(), 20, 20, juce::Justification::centred);

        g.setFont(juce::Font("Gill Sans", 11.0f, juce::Font::plain));
        g.drawText("Sample rate: " + juce::String(sampleRate) + " Hz",
                   bounds.getRight() - 100, 2, 90, 20,
                   juce::Justification::centred);
    }

private:
    float calculateYValue(bool linLog, double value,
                          const juce::Rectangle<float>& bounds)
    {
        return linLog ? bounds.getBottom()
                            - juce::jmap<float>(value, 0.0f, 2.0f, 0.0f,
                                                bounds.getHeight())
                      : juce::jmap<float>(
                          20.0f * std::log10(juce::jmax(value, 1e-12)), -60.0f,
                          60.0f, bounds.getBottom(), bounds.getY());
    }
};
...
public:
    ...
    void drawResponse(juce::Graphics& g, bool /*linLog*/) override
    {
        ...
        responsePath.startNewSubPath(
            bounds.getX(), bounds.getBottom() - values[0] * bounds.getHeight());
...
private:
    void drawPhaseGrid(juce::Graphics& g, const juce::Rectangle<float>& bounds)
    {
        ...
        std::vector<juce::String> referencePhases
            = {piString, piString + " / 2", "0", "-" + piString + " / 2",
               "-" + piString};
        ...
        {
            float y = bounds.getY()
                      + i * (bounds.getHeight() / (referencePhases.size() - 1));
            float offsetY
                = (i == 0 || i == referencePhases.size() - 1) ? -20.0f : -10.0f;

            g.setColour(juce::Colour(GRID_COLOUR));
            if (i != 0 && i != referencePhases.size() - 1)
            {
                g.drawHorizontalLine(static_cast<int>(y), bounds.getX(),
                                     bounds.getRight());
            }
            ...
            if (!i)
                g.drawText(referencePhases[i], 6, y, 20, 20,
                           juce::Justification::centredLeft);
            else
                g.drawText(referencePhases[i], 6, y + offsetY, 20, 20,
                           juce::Justification::centredLeft);
        }
    }
};
...
public:
            ...
            if (element.getType())
                poles.push_back(
                    std::polar(element.getMagnitude(),
                               MathConstants<double>::pi * element.getPhase()));
            else
                zeros.push_back(
                    std::polar(element.getMagnitude(),
                               MathConstants<double>::pi * element.getPhase()));
    ...
    juce::Rectangle<float> getBounds() { return bounds; }

    float getCentreX() { return bounds.getWidth() * 0.5 + 30 + 13; }

    float getCentreY() { return bounds.getHeight() * 0.5 + 430 + 7; }
...
private:
    ...
    void drawPlane(juce::Graphics& g)
    {
        ...
        g.drawLine(bounds.getX(), centerY, bounds.getRight(), centerY,
                   PLANE_LINE_THICKNESS);
        g.drawLine(centerX, bounds.getY(), centerX, bounds.getBottom(),
                   PLANE_LINE_THICKNESS);

        float radius = std::min(width, height) * 0.5f;
        g.drawEllipse(centerX - radius, centerY - radius, radius * 2,
                      radius * 2, CIRCLE_LINE_THICKNESS);
    ...
    void drawPolesAndZeros(juce::Graphics& g)
    {
        ...
        for (const auto& zero : zeros)
        {
            ...
            g.drawEllipse(x - radius, y - radius, radius * 2.0f, radius * 2.0f,
                          2.0f);
        }
...
{
public:
    DraggableElement(FilterElement e, int elNr, GaussianPlane* gp,
    ...
    void mouseDrag(const MouseEvent& event) override
    {
        ...
        juce::Point<int> planeCentre(gaussianPlane->getCentreX(),
                                     gaussianPlane->getCentreY());
        juce::Point<int> newRelativePosition(newX - planeCentre.x,
                                             planeCentre.y - newY);
        ...
        newY = juce::jlimit(
            juce::jlimit(0, planeCentre.y,
                         static_cast<int>(std::floor(planeCentre.y - yLimit))),
            static_cast<int>(planeCentre.y), newY);

        setBounds(newX - getWidth() * 0.5, newY - getHeight() * 0.5, getWidth(),
                  getHeight());

        toFront(true);

        std::complex<double> newPosition
            = getComplexFromCoordinates(getCentreX(), getCentreY());
    ...
    void updateElement(FilterElement e, int elNr, GaussianPlane* gp,
                       PolesAndZerosEQAudioProcessor* p)
    {
    ...
    float getCentreX() { return this->getX() + getWidth() * 0.5; }

    float getCentreY() { return this->getY() + getHeight() * 0.5; }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        case FilterElement::ZERO:
        {
            ...
            g.drawEllipse(2.5, 2.5, radius * 2.0f, radius * 2.0f,
        ...
        case FilterElement::POLE:
        {
            ...
            float radius = 6.0f;
            g.drawLine(centerX - radius, centerY - radius, centerX + radius,
                       centerY + radius, ELEMENT_LINE_THICKNESS);
            g.drawLine(centerX + radius, centerY - radius, centerX - radius,
                       centerY + radius, ELEMENT_LINE_THICKNESS);
        }
        break;
...
private:
    ...
    {
        x = (std::real(element) * (gaussianPlane->getWidth() * 0.5))
            + gaussianPlane->getCentreX() - getWidth() * 0.5;
        y = (-(std::imag(element)) * (gaussianPlane->getHeight() * 0.5))
            + gaussianPlane->getCentreY() - getHeight() * 0.5;
    }

    void setConjugateCoordinates()
    {
        conjX = x;
        conjY = (std::imag(element)) * (gaussianPlane->getHeight() * 0.5)
                + gaussianPlane->getCentreY() - getHeight() * 0.5;
    }
